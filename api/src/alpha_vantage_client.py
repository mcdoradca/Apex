...
class AlphaVantageClient:
    ...
    # === OPTYMALIZACJA: Limit 145 zapytań/minuta (Synchronizacja z Workerem) ===
    def __init__(self, api_key: str = API_KEY, requests_per_minute: int = 145, retries: int = 3, backoff_factor: float = 0.5):
        ...
        # Pacing: Równomierne rozłożenie zapytań (ok. 0.41s odstępu)
        self.request_interval = 60.0 / requests_per_minute
        self.request_timestamps = deque()
        
        # === OPTYMALIZACJA: Session Keep-Alive ===
        # Kluczowe dla API, które często obsługuje wiele małych zapytań od użytkownika
        self.session = requests.Session()

    def _rate_limiter(self):
        """
        Zaawansowany Rate Limiter typu 'Rolling Window'.
        Identyczny jak w Workerze dla spójności zachowania.
        """
        if not self.api_key:
             return
        ...
        # 1. Przesuń okno czasowe (usuń stare wpisy > 60s)
        while self.request_timestamps and (now - self.request_timestamps[0] > 60):
            self.request_timestamps.popleft()
            
        # 2. Sprawdź Twardy Limit
    ...
    def _make_request(self, params: dict):
        ...
        for attempt in range(self.retries):
            ...
            try:
                ...
                # Wykrywanie limitów w treści JSON
    ...
    def get_bulk_quotes(self, symbols: list[str]):
        """Używane przez API do szybkiego podglądu wielu cen (np. w portfelu)."""
        params = {
    ...
    def _parse_bulk_quotes_csv(self, csv_text: str, ticker: str) -> dict | None:
        """Przetwarza odpowiedź CSV (dla get_global_quote)."""
    ...
    def get_global_quote(self, symbol: str):
        """
        Pobiera dane 'quote' używając REALTIME_BULK_QUOTES.
        Kluczowa funkcja dla API/Frontendu (odświeżanie cen na żywo).
        """
    ...
    # === ENDPOINTY MAKRO I INNE (Dla spójności z Workerem) ===
