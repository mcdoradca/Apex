...
def _get_latest_value(data: dict, series_key: str, value_key: str) -> float | None:
    """Pobiera najnowszą wartość z serii danych technicznych."""
    if not data or series_key not in data:
        return None
    try:
        # Usunięcie prefiksów numerycznych z kluczy (np. "1. open" -> "open")
        cleaned_series = {date: {k.split('. ')[1]: v for k, v in values.items()} for date, values in data[series_key].items()}
        latest_date = sorted(cleaned_series.keys())[-1]
        return safe_float(cleaned_series[latest_date][value_key])
    except (IndexError, KeyError) as e:
        logger.warning(f"Could not get latest value for {series_key} / {value_key}: {e}")
        return None
...
def perform_full_analysis(ticker: str, api_client) -> dict:
    ...
    try:
        ...
        latest_day_data = {k.split('. ')[1]: v for k, v in daily_data['Time Series (Daily)'][dates[-1]].items()}
        prev_day_data = {k.split('. ')[1]: v for k, v in daily_data['Time Series (Daily)'][dates[-2]].items()}
        
        latest_price = safe_float(latest_day_data['close'])
        prev_price = safe_float(prev_day_data['close'])
