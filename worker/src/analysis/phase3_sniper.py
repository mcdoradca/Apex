...
# (Poniższe funkcje _find_... są nieaktywne, wywoływane tylko przez zakomentowany kod)
...
def find_end_of_day_setup(ticker: str, daily_df: pd.DataFrame) -> dict:
    ...
    # Stare metody generowania sygnałów (Breakout, EMA, Fib) zostały
    # celowo wyłączone, aby przygotować system pod wdrożenie
    # nowej strategii opartej na modelu AQM (Apex Quality Model).
    # Ta funkcja celowo nie generuje już żadnych sygnałów PENDING.
    # ==================================================================
    return {"signal": False, "reason": "DEKONSTRUKCJA: Stare strategie EOD celowo wyłączone."}
    
    # PONIŻEJ ZNAJDUJE SIĘ ORYGINALNY, WYGASZONY KOD:
...
def run_tactical_planning(session: Session, qualified_data: List[Tuple[str, pd.DataFrame]], get_current_state, api_client: AlphaVantageClient):
    ...
    for ticker, daily_df in qualified_data:
        try:
            ...
            # ==================================================================
            # === DEKONSTRUKCJA (KROK 1) ===
            # Poniższa logika zapisu do bazy danych zostanie wykonana tylko
            # jeśli find_end_of_day_setup zwróci "signal": True.
            # Ponieważ zmieniliśmy tę funkcję, aby zawsze zwracała False,
            # ten blok kodu jest teraz skutecznie wyłączony.
            # ==================================================================
            if trade_setup.get("signal"):
                # Ten kod jest teraz niedostępny, ponieważ trade_setup["signal"] jest zawsze False
                successful_setups += 1
                stmt = text("""...""")
                params = {...}
                ...
                # Logika, która będzie teraz wykonywana dla każdego tickera:
    ...
    # Zmieniamy log końcowy, aby odzwierciedlał dekonstrukcję
    append_scan_log(session, f"Faza 3 (Skaner EOD) zakończony. Stare strategie wyłączone. Znaleziono {successful_setups} setupów.")
...
def monitor_fib_confirmations(session: Session, api_client: AlphaVantageClient):
    ...
    """
    logger.info("[Monitor Fib H1] DEKONSTRUKCJA: Monitor celowo wyłączony.")
    return
    ...
    # ... (cała logika monitora Fib została wyłączona) ...
    # except Exception as e:
    #     logger.error(f"Krytyczny błąd w monitorze Fib H1: {e}", exc_info=True)
    #     session.rollback()
# ==================================================================
# KONIEC DEKONSTRUKCJI MONITORA FIB
# ==================================================================
...
def monitor_entry_triggers(session: Session, api_client: AlphaVantageClient):
    ...
    ani PENDING pod kątem SL/TP, ponieważ stara formuła jest
    całkowicie wygaszona.
    """
    logger.info("[Monitor Szybki] DEKONSTRUKCJA: Monitor (SL/TP) celowo wyłączony.")
    return
    ...
    # logger.info("Running Real-Time Entry Trigger Monitor (Fast - SL/TP ONLY)...")
    ...
    # logger.info(f"Monitoring {len(tickers_to_monitor)} tickers for SL/TP using 1 bulk request.")
    ...
    #     # Iterujemy po sygnałach i sprawdzamy TYLKO SL i TP
    ...
    #         # (TA LOGIKA POZOSTAJE AKTYWNA, aby zamykać istniejące pozycje 'ACTIVE')
    #         # ==================================================================
    #         # Sprawdzamy tylko sygnały ACTIVE
    #         if signal_row.status == 'ACTIVE' and take_profit_price is not None and current_price >= take_profit_price:
    ...
    #         # (TA LOGIKA POZOSTAJE AKTYWNA, aby chronić *wszystkie* pozycje)
    #         # ==================================================================
    #         if stop_loss_price is not None and current_price <= stop_loss_price:
    #             # CENA JEST PONIŻEJ STOP LOSSA!
    #             # Ta logika unieważni zarówno sygnały PENDING (jeśli cena spadnie poniżej SL)
    #             # jak i ACTIVE (zamykając pozycję).
    ...
    #         # ==================================================================
    #         # === DEKONSTRUKCJA (KROK 2) ===
    #         # WSZYSTKIE PONIŻSZE BLOKI LOGICZNE (ZUŻYCIE, AKTYWACJA BREAKOUT, AKTYWACJA FIB)
    #         # ZOSTAŁY CELOWO USUNIĘTE, ABY ZATRZYMAĆ AKTYWACJĘ STARYCH STRATEGII.
    #         # ==================================================================
