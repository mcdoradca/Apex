...
def _pre_calculate_metrics_live(daily_df: pd.DataFrame, insider_df: pd.DataFrame, news_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Fix: Handle division by zero / inf for volume
    ...
    if not news_df.empty:
        ...
        # Fix: Handle division by zero / inf for news
...
def run_h3_live_scan(session: Session, candidates: List[str], api_client: AlphaVantageClient):
    ...
    for i, ticker in enumerate(candidates):
        ...
        try:
            ...
            # (Opcjonalnie BBands - backtest używa, więc tu też pobieramy dla spójności cache)
            ...
            if (current_aqm > current_thresh) and (current_m < H3_M_SQ_THRESHOLD):
                ...
                # ZAPIS DO BAZY (Z konwersją float!)
                ...
                if not existing:
                    # === POPRAWKA TUTAJ: Konwersja float(...) ===
                    new_signal = models.TradingSignal(
                        ticker=ticker,
                        status='PENDING',
                        generation_date=datetime.now(timezone.utc),
                        entry_price=float(ref_price),           # <-- FIX
                        stop_loss=float(stop_loss),             # <-- FIX
                        take_profit=float(take_profit),         # <-- FIX
                        risk_reward_ratio=float(H3_TP_MULT/H3_SL_MULT), # <-- FIX
                        entry_zone_top=float(ref_price + (0.5 * atr)),  # <-- FIX
                        entry_zone_bottom=float(ref_price - (0.5 * atr)), # <-- FIX
