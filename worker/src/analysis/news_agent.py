...
# ==================================================================
# GŁÓWNY CYKL AGENTA (WERSJA AV NATIVE + BATCHING)
# ==================================================================
...
def run_news_agent_cycle(session: Session, api_client: object):
    ...
    Wersja z poprawionym BATCHINGIEM (obsługuje >50 tickerów).
    """
    ...
    try:
        ...
        all_tickers.sort() # Sortowanie dla porządku

        if not all_tickers:
            return

        # LOGOWANIE STARTU DO UI
        append_scan_log(session, f"Agent Newsowy: Start cyklu. Monitoruję {len(all_tickers)} tickerów (w paczkach po 50).")

        processed_count = 0
        alerts_count = 0
        
        # === BATCHING LOOP (NAPRAWIONE) ===
        # Dzielimy listę na paczki po 50 sztuk (limit Alpha Vantage na jeden request)
        BATCH_SIZE = 50
        batches = [all_tickers[i:i + BATCH_SIZE] for i in range(0, len(all_tickers), BATCH_SIZE)]

        for i, batch in enumerate(batches):
            # logger.info(f"Agent Newsowy: Przetwarzanie paczki {i+1}/{len(batches)}...")
            
            ticker_string = ",".join(batch)
            
            # 3. Zapytanie do Alpha Vantage
            ...
            # Krótki sleep między paczkami, żeby dać odetchnąć API (mimo rate limitera w kliencie)
            if len(batches) > 1:
                time.sleep(0.5)
            ...
            # 4. Przetwarzanie Feed-u dla danej paczki
            for item in news_data.get('feed', []):
                headline = item.get('title', 'No Title')
                # summary = item.get('summary', 'No Summary') # Nieużywane
                url = item.get('url', '#')
                ...
                for ts_data in ticker_sentiment_list:
                    ...
                    # Czy nas ten ticker obchodzi? (Musi być w naszej ogólnej liście)
                    ...
                    # === LOGIKA DECYZYJNA (TYLKO METRYKI AV) ===
                    ...
                    if should_alert:
                        ...
                        # Formatowanie wiadomości (HTML dla Telegrama w utils.py)
                        # clean_msg idzie do Telegrama
                        clean_msg = (
