...
class AlphaVantageClient:
    ...
    def get_bulk_quotes(self, symbols: list[str]):
        ...
        self._rate_limiter()
        # --- OSTATECZNA POPRAWKA ---
        # 1. Zmieniono 'symbols' na 'symbol' zgodnie z dokumentacją.
        # 2. Dodano 'entitlement=delayed' - kluczowy brakujący parametr.
        params = {
        ...
        for attempt in range(self.retries):
            try:
                ...
                # Ulepszone sprawdzanie błędów, szukamy też komunikatu o "sample data"
...
class AlphaVantageClient:
    ...
    def _make_request(self, params: dict):
        ...
        # --- ZMIANA GLOBALNA ---
        # Dodajemy 'entitlement=delayed' do wszystkich standardowych zapytań,
        # ponieważ jest to wymagane dla Twojego planu premium.
        params['entitlement'] = 'delayed'
        ...
        for attempt in range(self.retries):
            try:
                ...
                if "Note" in data:
                    logger.warning(f"API Note for {params.get('symbol')}: {data['Note']}.")
                    return None # Traktujemy "note" jako potencjalny błąd (np. limit zapytań)
                return data
            except (requests.exceptions.RequestException, json.JSONDecodeError) as e:
                logger.error(f"Request failed (attempt {attempt + 1}/{self.retries}): {e}")
                if attempt < self.retries - 1:
