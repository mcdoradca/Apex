...
def _run_schema_and_index_migration(session: Session):
    ...
    try:
        ...
        # Sprawdź, czy DOKŁADNIE TEN indeks już istnieje
        indexes = inspector.get_indexes('trading_signals')
        index_exists = any(idx['name'] == index_name for idx in indexes)

        if not index_exists:
            logger.warning(f"CRITICAL FIX: Index '{index_name}' not found. Creating it now...")
            try:
                # Usunięcie starego, potencjalnie błędnego ograniczenia, jeśli istniało
                session.execute(text("ALTER TABLE trading_signals DROP CONSTRAINT IF EXISTS trading_signals_ticker_key;"))
                
                # Stworzenie nowego, poprawnego częściowego indeksu unikalnego
                create_index_sql = text(f"""
                    CREATE UNIQUE INDEX IF NOT EXISTS {index_name}
                    ON trading_signals (ticker)
                    WHERE status IN ('ACTIVE', 'PENDING');
                ...
                session.execute(create_index_sql)
                session.commit()
                logger.info(f"Successfully created partial unique index '{index_name}'.")
            except Exception as e:
                session.rollback()
                logger.error(f"Failed to create index {index_name}: {e}", exc_info=True)
        else:
            logger.info(f"Index '{index_name}' already exists. No migration needed.")
