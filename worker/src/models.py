from sqlalchemy import ( # <<< POPRAWIONO 'rom' na 'from'
    Column, String, VARCHAR, TIMESTAMP, NUMERIC, BIGINT, DATE,
    Boolean, INTEGER, TEXT, ForeignKey, Index, func, text # Dodano 'text' dla spójności z API
)
# Import PG_TIMESTAMP bezpośrednio
from sqlalchemy.dialects.postgresql import TIMESTAMP as PG_TIMESTAMP, JSONB
# Importujemy 'Base' z LOKALNEGO pliku database.py Workera
...
class TradingSignal(Base):
    ...
    status = Column(VARCHAR(50), default='PENDING', index=True)
    ...
    __table_args__ = (
        Index(
            ...
            'ticker',
            unique=True,
            # Używamy zaimportowanej funkcji 'text'
...
# (Te modele są teraz obecne, zapewniając spójność z API)
...
class TransactionHistory(Base):
    ...
    transaction_type = Column(VARCHAR(10), nullable=False, index=True, comment="'BUY' lub 'SELL'")
    ...
    transaction_date = Column(PG_TIMESTAMP(timezone=True), server_default=func.now(), index=True, comment="Data i czas transakcji")
    profit_loss_usd = Column(NUMERIC(14, 2), nullable=True, comment="Zrealizowany zysk/strata w USD dla transakcji sprzedaży")


# === NOWA TABELA DLA BUFOROWANIA CEN ===
