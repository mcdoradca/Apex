...
// Importy logiki - teraz na pewno poprawne
import { 
    setUI, 
...
// Obiekt Logic dla łatwiejszego debugowania w konsoli (window.Logic)
const Logic = {
    setUI, 
    showDashboard, showPortfolio, showTransactions, showAgentReport,
    pollWorkerStatus, refreshSidebarData, pollSystemAlerts,
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        function startApp() {
            ...
            // Status Online od razu, api.js zmieni go jeśli coś pójdzie nie tak
            if (UI.apiStatus) {
        ...
        // Globalny Event Listener dla wszystkich kliknięć w głównej treści
        if (UI.mainContent) {
        ...
        // Obsługa Modali
        if (UI.h3LiveModal.cancelBtn) UI.h3LiveModal.cancelBtn.addEventListener('click', Logic.hideH3LiveParamsModal);
        if (UI.h3LiveModal.startBtn) UI.h3LiveModal.startBtn.addEventListener('click', Logic.handleRunH3LiveScan);
    
        if (UI.signalDetails && UI.signalDetails.closeBtn) UI.signalDetails.closeBtn.addEventListener('click', Logic.hideSignalDetails);
        ...
        // Auto-Login Loop (Czeka na "obudzenie" API)
            ...
            try {
                ...
                if (status && status.status && status.status.includes("running")) {
                    ...
                    if (UI.loginButton) {
                        ...
                        UI.loginButton.classList.remove('cursor-not-allowed');
                        UI.loginButton.classList.add('hover:bg-sky-500');
                    }
                } else {
                    if (UI.loginStatusText) UI.loginStatusText.textContent = 'Wybudzanie serwera... Proszę czekać (ok. 30s)';
                }
            } catch (e) {
                if (UI.loginStatusText) UI.loginStatusText.textContent = 'Łączenie z chmurą...';
            }
        }, 2000);
