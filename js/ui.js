...
console.log(">>> [UI] Ładowanie modułu ui.js...");
...
try {
    ...
    style.textContent = `
        ...
        @keyframes heartbeat-busy { 0% { box-shadow: 0 0 0 0 rgba(234, 179, 8, 0.7); transform: scale(1); } 25% { transform: scale(1.03); } 50% { box-shadow: 0 0 0 15px rgba(234, 179, 8, 0); transform: scale(1); } 100% { box-shadow: 0 0 0 0 rgba(234, 179, 8, 0); transform: scale(1); } }
    ...
    console.log(">>> [UI] CSS wstrzyknięty pomyślnie.");
} catch (e) {
    console.error(">>> [UI] BŁĄD wstrzykiwania CSS:", e);
}
...
const renderPhase1List = (candidates) => {
    try {
        ...
        console.error("Błąd renderPhase1List:", e);
        return `<p class="text-xs text-red-500">Błąd F1</p>`;
    }
};
...
const renderPhase3List = (signals) => {
    try {
            ...
            if (s.notes && s.notes.includes("SCORE:")) {
                try {
                    ...
                    if (parts.length > 1) {
                        ...
                        if (strat.name === 'FLUX') scoreBg = "bg-emerald-900/30 text-emerald-300 flux-pulse";
        ...
        console.error("Błąd renderPhase3List:", e);
        return `<p class="text-xs text-red-500">Błąd F3</p>`;
    }
};
...
const renderPhase4View = (candidates) => {
    try {
        ...
        console.error("Błąd renderPhase4View:", e);
        return `<p class="text-red-500">Błąd F4: ${e.message}</p>`;
    }
};

const renderPhase5View = (poolData) => {
    console.log(">>> [UI] renderPhase5View - Start. Data:", poolData);
    try {
        const activeSlots = Array.isArray(poolData) ? poolData.slice(0, 8) : [];
        ...
        const slotsHtml = activeSlots.map((item, index) => {
            ...
            // SL / TP / RR extraction
            const sl = item?.stop_loss || 0;
            const tp = item?.take_profit || 0;
            let rrText = "---";
            ...
            if (sl > 0 && tp > 0 && price > 0) {
                ...
                if (risk > 0.0001) {
                     rrText = (reward / risk).toFixed(1) + "R";
                }
            }
            ...
            <div class="pool-slot ${cardState} relative overflow-hidden group">
                ...
                <div class="flex flex-col gap-1 my-2 px-2 py-1 bg-black/20 rounded border border-white/5">
                    ...
                    <div class="flex justify-between items-center pt-1 mt-1 border-t border-white/10">
                        ...
                        <span class="text-[10px] font-bold text-yellow-500 font-mono">${rrText}</span>
                    </div>
                </div>
        ...
        const emptySlotsHtml = Array.from({length: Math.max(0, emptySlotsCount)}, (_, i) => `
        ...
        <div id="phase5-monitor-view" class="max-w-7xl mx-auto">
            ...
            <div class="mt-4 p-3 bg-[#161B22] border border-gray-700 rounded text-xs text-gray-400 flex items-center justify-between">
                ...
                <div class="font-mono text-gray-600">v5.3 Safe View</div>
            </div>
        </div>`;

    } catch (e) {
        console.error("Krytyczny błąd renderowania Fazy 5:", e);
        return `<div class="p-4 bg-red-900/30 border border-red-500 text-red-200 rounded">Błąd widoku Fazy 5: ${e.message}</div>`;
    }
};
...
const renderH3SignalsPanel = (signals, quotes = {}) => {
    try {
        ...
        return `<p class="text-red-500">Błąd H3: ${e.message}</p>`;
    }
};
...
const renderPhaseXView = (candidates) => {
    try {
        ...
        return `<p class="text-red-500">Błąd FX: ${e.message}</p>`;
    }
};
...
const renderPortfolio = (holdings, quotes) => {
    try {
        ...
        return `<p class="text-red-500">Błąd portfela: ${e.message}</p>`;
    }
};
...
const renderTransactions = (txHistory) => {
    try {
        ...
        console.error("Błąd renderowania transakcji:", e);
        return `<p class="text-red-500">Błąd transakcji: ${e.message}</p>`;
    }
};
...
const renderAgentReport = (report) => {
    try {
        ...
        console.error("Błąd renderowania raportu:", e);
        return `<p class="text-red-500">Błąd raportu: ${e.message}</p>`;
    }
};
...
const renderOptimizationResults = (job) => {
    try {
        ...
        console.error("Błąd renderowania wyników:", e);
        return `<p class="text-red-500">Błąd wyników: ${e.message}</p>`;
    }
};
...
export const ui = {
    init: () => {
        console.log(">>> [UI] Inicjalizacja elementów DOM...");
        try {
            const get = (id) => {
            ...
            console.log(">>> [UI] Inicjalizacja zakończona.");
            return {
            ...
            console.error(">>> [UI] KRYTYCZNY BŁĄD W UI.INIT:", e);
            // Awaryjny obiekt UI, aby nie wywalić app.js
            return { loginScreen: document.getElementById('login-screen'), dashboardScreen: document.getElementById('dashboard') };
        }
    },
    renderers: renderers
};
