...
const renderPhase5View = (poolData) => {
    try {
        ...
        const slotsHtml = activeSlots.map((item, index) => {
            ...
            // === BEZPIECZNA IMPLEMENTACJA SL/TP (NOWOŚĆ) ===
            const sl = (item && typeof item.stop_loss === 'number' && item.stop_loss > 0) ? item.stop_loss : 0;
            const tp = (item && typeof item.take_profit === 'number' && item.take_profit > 0) ? item.take_profit : 0;
            
            let rrText = "---";
            if (sl > 0 && tp > 0 && price > 0) {
                const risk = Math.abs(price - sl);
                const reward = Math.abs(tp - price);
                if (risk > 0.0001) rrText = (reward / risk).toFixed(1) + "R";
            }
            ...
            <div class="pool-slot ${cardState} relative overflow-hidden group">
                ...
                <!-- ZMODYFIKOWANA SEKCJA: RYZYKO (SL/TP) -->
        ...
        console.error("Błąd renderowania widoku Fazy 5:", e);
        return `<div class="p-4 text-red-500">Błąd widoku F5</div>`;
    }
};
