...
export const renderers = {
    ...
    agentReport: (report) => {
        ...
        const backtestSection = `<div class="bg-[#161B22] p-6 rounded-lg shadow-lg border border-gray-700"><h4 class="text-lg font-semibold text-gray-300 mb-3">Uruchom Nowy Test Historyczny</h4><p class="text-sm text-gray-500 mb-4">Wpisz rok (np. 2010), aby przetestować strategie na historycznych danych dla tego roku.</p><div class="flex items-start gap-3 mb-4"><input type="number" id="backtest-year-input" class="modal-input w-32 !mb-0" placeholder="YYYY" min="2000" max="${new Date().getFullYear()}"><button id="run-backtest-year-btn" class="modal-button modal-button-primary flex items-center flex-shrink-0"><i data-lucide="play" class="w-4 h-4 mr-2"></i> Uruchom Test</button></div><div class="border-t border-gray-700 pt-3"><button id="toggle-h3-params" class="text-xs text-sky-400 flex items-center hover:text-sky-300 transition-colors mb-3"><i data-lucide="settings-2" class="w-3 h-3 mr-1"></i> Zaawansowana Konfiguracja H3 (Symulator) <i data-lucide="chevron-down" id="h3-params-icon" class="w-3 h-3 ml-1 transition-transform"></i></button><div id="h3-params-container" class="hidden grid grid-cols-1 md:grid-cols-3 gap-3 bg-gray-800/30 p-3 rounded-md border border-gray-700/50"><div><label class="text-[10px] text-gray-400 uppercase font-bold">Percentyl AQM (Domyślny: 0.95)</label><input type="number" id="h3-param-percentile" class="modal-input text-xs !py-1 !h-8" placeholder="0.95" step="0.01" min="0.80" max="0.99"></div><div><label class="text-[10px] text-gray-400 uppercase font-bold">Próg Masy m² (Domyślny: -0.5)</label><input type="number" id="h3-param-mass" class="modal-input text-xs !py-1 !h-8" placeholder="-0.5" step="0.1" max="0"></div><div><label class="text-[10px] text-gray-400 uppercase font-bold">Mnożnik TP (ATR) (Domyślny: 5.0)</label><input type="number" id="h3-param-tp" class="modal-input text-xs !py-1 !h-8" placeholder="5.0" step="0.5"></div><div><label class="text-[10px] text-gray-400 uppercase font-bold">Mnożnik SL (ATR) (Domyślny: 2.0)</label><input type="number" id="h3-param-sl" class="modal-input text-xs !py-1 !h-8" placeholder="2.0" step="0.5"></div><div><label class="text-[10px] text-gray-400 uppercase font-bold">Max Hold (Dni) (Domyślny: 5)</label><input type="number" id="h3-param-hold" class="modal-input text-xs !py-1 !h-8" placeholder="5" step="1"></div><div><label class="text-[10px] text-gray-400 uppercase font-bold">Nazwa Setupu (Suffix)</label><input type="text" id="h3-param-name" class="modal-input text-xs !py-1 !h-8" placeholder="CUSTOM_TEST_1"></div></div></div><div id="backtest-status-message" class="text-sm mt-3 h-4"></div></div>`;
