...
style.textContent = `
    ...
    @keyframes heartbeat-busy {
        0% { box-shadow: 0 0 0 0 rgba(234, 179, 8, 0.7); transform: scale(1.03); }
...
// =========================================================================
// === DEFINICJE FUNKCJI RENDERUJĄCYCH (Jako stałe - BEZPIECZNIE) ===
// =========================================================================
    ...
    try {
        ...
        const slotsHtml = activeSlots.map((item, index) => {
            ...
            if (score >= 65) {
                ...
                if (elast < -1.0) actionText = "DIP BUY"; else if (elast > 0.5) actionText = "BREAKOUT";
            ...
            <div class="pool-slot ${cardState} relative overflow-hidden group">
                ...
                <div class="flux-action-text ${actionColor} py-2">${actionText}</div>
        ...
        <div id="phase5-monitor-view" class="max-w-7xl mx-auto">
            ...
            <div class="mb-4"><div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">${slotsHtml}${emptySlotsHtml}</div></div>
            <div class="mt-4 p-3 bg-[#161B22] border border-gray-700 rounded text-xs text-gray-400 flex items-center justify-between">
                ...
                <div class="font-mono text-gray-600">v5.4 Fixed View</div>
            </div>
        </div>`;

    } catch (e) {
        console.error("Błąd renderowania widoku F5:", e);
        return `<div class="p-4 bg-red-900/50 text-red-200 rounded">Błąd widoku F5: ${e.message}</div>`;
    }
};
