...
const renderPhase1List = (candidates) => {
    try {
        ...
        return `<p class="text-xs text-red-500 p-2">Błąd F1</p>`;
    }
};
...
const renderPhase3List = (signals) => {
    try {
        ...
        return signals.map(s => {
            ...
            if (s.status === 'ACTIVE' && scoreVal >= 80) { playTacticalAlert(s.ticker, (scoreVal * 100).toFixed(0)); }
        ...
        return `<p class="text-xs text-red-500 p-2">Błąd F3</p>`;
    }
};
...
const renderPhase5View = (poolData) => {
    ...
    const slotsHtml = activeSlots.map((item, index) => {
        // 2. EKSTRAKCJA I SPRAWDZANIE TYPÓW (ABY UNIKNĄĆ BŁĘDÓW TOFIXED)
        ...
        // 3. LOGIKA SL/TP (NOWA, ALE BEZPIECZNA)
        ...
        // 4. LOGIKA KARTY (Z DEKLARACJĄ ZMIENNYCH NA POCZĄTKU)
    ...
    <div id="phase5-monitor-view" class="max-w-7xl mx-auto">
        ...
        <div class="mt-4 p-3 bg-[#161B22] border border-gray-700 rounded text-xs text-gray-400 flex items-center justify-between">
            ...
            <div class="font-mono text-gray-600">v5.7 Risk Aware</div>
        </div>
    </div>`;
};
